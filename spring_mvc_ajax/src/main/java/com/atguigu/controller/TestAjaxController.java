 ...
 * c>在处理请求的控制器方法的形参位置，直接设置json格式的请求参数要转换java类型的形参，使用@RequestBody注解表示的接口即可
 * 3.@ResponseBody将所表示的控制器方法的返回值作为报文的响应体响应到浏览器
 * 4.使用@ResponseBody注解响应浏览器json格式的数据
 * a>导入jackson依赖
 * b>在SpringMVC的配置文件中设置<mvc:annotation-driven/>
 * c>将需要转换为json字符串的java 对象直接作为控制器方法的返回值，使用@ResponseBody注解标识控制器得方法
